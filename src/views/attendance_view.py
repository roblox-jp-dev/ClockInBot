...
class ProjectSelectModal(ui.Modal):
    """勤務開始時のプロジェクト選択モーダル"""
    
    def __init__(self, projects: List[Dict[str, Any]], locale: str):
        super().__init__(title=I18n.t("modal.project", locale))
    ...
    async def on_submit(self, interaction: Interaction):
        # このモーダルの結果は呼び出し元のattendance_viewで処理
        pass
...
async def handle_start_work_button(interaction: Interaction):
    """勤務開始ボタンの処理"""
    ...
    if not channel_mapping:
        await interaction.followup.send(I18n.t("common.error", message="Channel not found"), ephemeral=True)
        return
    ...
    if active_session:
        # 既に勤務中の場合はエラー
        await interaction.followup.send(I18n.t("attendance.alreadyStarted"), ephemeral=True)
        return
    
    # サーバーのロケールを取得
    # 実際の実装では、データベースからサーバー設定を取得してロケールを設定
    locale = "ja"
    ...
    # プロジェクト選択モーダルを表示
    modal = ProjectSelectModal(projects, locale)
    ...
    # 選択されたプロジェクトIDを取得
    selected_project_id = int(modal.project_select.values[0])
    
    # プロジェクト情報を取得
    project = await ProjectRepository.get_project(selected_project_id)
    
    # 勤務開始を記録
    session = await AttendanceRepository.start_session(guild_user_id, selected_project_id)
...
async def handle_end_work_button(interaction: Interaction):
    """勤務終了ボタンの処理"""
    await interaction.response.defer(ephemeral=True)
    ...
    if not active_session:
        # 勤務していない場合はエラー
        await interaction.followup.send(I18n.t("attendance.notStarted"), ephemeral=True)
        return
    ...
    if require_modal:
        ...
        # 入力された要約を取得
        end_summary = modal.summary.value
    ...
    user = interaction.user
    await interaction.followup.send(
        I18n.t("attendance.end", locale, username=user.display_name, duration=duration_str),
        ephemeral=True
    )
